...
bool Log::Init(const char* programName, const char* logDir)
{
    ...
    if (access(logDir, 0) != 0) {
        if (mkdir(logDir, S_IRUSR | S_IWUSR | S_IXUSR | S_IRGRP | S_IROTH) != 0) {
            std::cout << programName << " failed to create log dir" << logDir << ", errno=" << errno
                      << std::endl;
            return false;
        }
    }
...
void Log::Destory() { google::ShutdownGoogleLogging(); }

void Log::Writer(const char* data, size_t size) { EMDB_LOG(E) << data; }